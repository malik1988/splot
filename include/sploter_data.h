...
namespace splot
{
  struct sploter_data
  {
     ...
     */
    explicit sploter_data(std::deque<float> &&xseries, std::function<float(float)> func, std::string_view name = "");
     ...
     * @param name curve name, empty will auto generate by index
     */
    explicit sploter_data(std::deque<float> &&xseries, std::deque<float> &&yseries, std::string_view name = "");
