    ...
    if (show_axes_mark_) {
        ...
        //------------------axis numbers------------------------
        {
            ...
            for (int i = 0; i <= x_divs; ++i) {
                ...
                text << std::fixed << std::setprecision(1) << val;
                render->draw_text(
                    x + width / x_divs * i, y_zero + (margin / 2), text.str().c_str());
            }
            render->end_line_style();
        }
    }
...
void sploter::draw_curves(irender* render)
{
    ...
    int             index       = 0;
    for (auto& [xs, ys, color, visible, name, area] : curves_) {
        ++index;
        if (show_legend_) {
            // draw legend box
            // select button
            area.x              = x_ + padding + (index - 1) * margin;
            area.xr             = x_ + padding + button_size + (index - 1) * margin;
            area.y              = y_ - padding;
            area.yb             = y_;
            const auto& btn_x   = area.x;
            const auto& btn_x_r = area.xr;
            const auto& btn_y   = area.y;
            const auto& btn_y_b = area.yb;
            if (area.visible) {
            ...
            if (name.empty()) {
                ...
                ss << "curve" << index;
                render->draw_text(
                    x_ + padding + (index - 1) * margin, y_ - padding, ss.str().c_str());
            }
            else {
...
bool sploter::mouse_click(irender* render, float x, float y, const bool down, const mouse_key key)
{
    bool refresh = false;
    if (key == mouse_key::left) {
        ...
        if (down) {
            for (auto& curve : curves_) {
                const auto& [a_x, a_y, a_xr, a_yb, visible] = curve.area;
                if (x >= a_x && x <= a_xr && y >= a_y && y <= a_yb) {
...
bool sploter::handle_events(irender* render, float x, float y, cursor_tracking tracking)
{
    ...
    case cursor_tracking::entering: cursor_over_ = true; break;
    case cursor_tracking::leaving: cursor_over_ = false; break;
    default: break;
    }

    for (auto& curve : curves_) {
        auto& [a_x, a_y, a_xr, a_yb, visible] = curve.area;
        if (x >= a_x && x <= a_xr && y >= a_y && y <= a_yb) {
            visible = true;
            refresh = true;
        }
...
void sploter::plot(std::deque<float> xseries, std::function<float(float)> func,
...
std::tuple<float, float, float, float> sploter::calc_range(float min, float max, float len)
{
    ...
    else if (max <= 0) {   // all negetive values
